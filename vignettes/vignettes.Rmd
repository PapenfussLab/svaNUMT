...
`VCF` object to remove unwanted calls.Details of `VCF` objects can be found by 
...
gr.circos <- gr %>% 
	mutate(to.gr=GRanges(seqnames(partner(gr)),ranges(partner(gr))))
seqlevels(gr.circos)<-seqlevels(hg19sub)
```
We can then plot the breakpoints agains referece genomes.
```{r ggbio}
