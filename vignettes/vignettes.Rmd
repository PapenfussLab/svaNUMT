...
To demonstrate `findBreakpointOverlaps()` function, we construct a toy `GRanges` object `region.gr` using two partnered breakpoints from `gr`. The output `Hits` object reports the index of overlapping breakpoints (`queryHits` and `subjectHits`), and overlapping distances (`sizeerror`, `localbperror` and `remotebperror`). 
```{r}
...
seqlevels(gr)<-sub(pattern = 'chr', replacement = '', seqlevels(gr))
